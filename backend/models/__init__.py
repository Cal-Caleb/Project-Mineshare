...
class ServerEventStatus(enum.StrEnum):
    STARTED = "started"
    SUCCESS = "success"
    FAILED = "failed"
...
class User(Base):
    ...
    mc_username: Mapped[str | None] = mapped_column(String(16), nullable=True, unique=True)
    ...
    mods_added: Mapped[list["Mod"]] = relationship(back_populates="added_by_user", foreign_keys="Mod.added_by_id")
...
class Mod(Base):
    ...
    curse_project_id: Mapped[int | None] = mapped_column(Integer, nullable=True, unique=True)
    ...
    status: Mapped[ModStatus] = mapped_column(Enum(ModStatus), default=ModStatus.PENDING_VOTE, nullable=False)
    ...
    added_by_user: Mapped["User"] = relationship(back_populates="mods_added", foreign_keys=[added_by_id])
...
class Vote(Base):
    ...
    status: Mapped[VoteStatus] = mapped_column(Enum(VoteStatus), default=VoteStatus.PENDING, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=_utcnow)
    ...
    initiated_by_user: Mapped["User"] = relationship(back_populates="votes_initiated", foreign_keys=[initiated_by_id])
    resolved_by_user: Mapped["User | None"] = relationship(foreign_keys=[resolved_by_id])
    ballots: Mapped[list["VoteBallot"]] = relationship(back_populates="vote", cascade="all, delete-orphan")
...
class VoteBallot(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    vote_id: Mapped[int] = mapped_column(Integer, ForeignKey("votes.id"), nullable=False)
...
class ModUpload(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    mod_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("mods.id"), nullable=True)
    ...
    uploaded_by_user: Mapped["User"] = relationship(back_populates="uploads", foreign_keys=[uploaded_by_id])
...
class ServerEvent(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    event_type: Mapped[ServerEventType] = mapped_column(Enum(ServerEventType), nullable=False)
    status: Mapped[ServerEventStatus] = mapped_column(Enum(ServerEventStatus), nullable=False)
    details: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class ServerHeartbeat(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    bucket: Mapped[datetime] = mapped_column(DateTime, nullable=False, unique=True, index=True)
